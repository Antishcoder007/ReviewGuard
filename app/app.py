...
#             final_df = predict_all(processed_df)

#             # Save prediction output
#             output_id = str(uuid.uuid4())
#             output_file = os.path.join(OUTPUT_FOLDER, f"predicted_{output_id}.csv")
#             final_df.to_csv(output_file, index=False)

#             # Prepare stats for dashboard
#             fake_pct = round(final_df['predicted_fake'].mean() * 100, 2)
#             sentiment_counts = final_df['predicted_sentiment'].value_counts().to_dict()

#             # Rating correction
#             avg_before = round(final_df['rating'].mean(), 2)
#             avg_after = round(final_df['corrected_rating'].mean(), 2)
...
#                                    avg_after=avg_after,
#                                    download_file=f"predicted_{output_id}.csv")
